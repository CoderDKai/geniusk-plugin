...
2. **查询最新技术信息**（使用 MCP 工具）：
   - 使用 **Context7** 查询候选技术栈的最新文档和版本特性
     - 调用 `mcp__context7__resolve-library-id` 获取库 ID
     - 调用 `mcp__context7__query-docs` 查询最新 API 和最佳实践
   - 使用 **Exa** 搜索技术对比和社区评价
     - 使用 `mcp__exa__web_search_exa` 查询最新技术趋势
     - 使用 `mcp__exa__get_code_context_exa` 查找代码示例

3. 提出 2-3 种技术栈方案，每种包含：
   - 语言/框架/版本（基于最新查询结果）
   - 核心依赖库（验证最新版本兼容性）
   - 测试框架
   - 适用场景
...
**注意**：
- 技术选型的方案对比过程不写入 design.md，只将最终选定的技术栈写入文档
- 查询时优先使用 Context7 获取官方文档，用 Exa 补充社区观点
- 根据候选技术数量合理安排查询，确保所有关键技术都有充分调研
